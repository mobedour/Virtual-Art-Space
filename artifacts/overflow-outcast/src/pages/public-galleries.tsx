    ...
    <PublicLayout>
      <div className="container mx-auto px-6 py-12">
        <div className="mb-16 text-center max-w-2xl mx-auto">
          <div className="inline-block mb-4 px-3 py-1 border border-primary/30 bg-primary/5 text-primary font-sans text-[10px] tracking-[0.2em] uppercase rounded-sm">
            Open Exhibitions
          </div>
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {galleries?.map((gallery, i) => (
              <Card key={gallery.id} className="bg-card/40 border-border/50 backdrop-blur-sm rounded-sm hover:bg-card/70 transition-all hover:border-primary/40 group animate-in fade-in slide-in-from-bottom-8" style={{ animationDelay: `${i * 100}ms` }}>
                <div className="aspect-[4/3] bg-background/80 border-b border-border/50 relative overflow-hidden flex items-center justify-center">
                  ...
                  <div className="text-center z-10 p-6 transform group-hover:scale-[1.02] transition-transform duration-500">
                    <h3 className="font-display font-semibold text-xl text-foreground mb-2 group-hover:text-primary transition-colors">{gallery.title}</h3>
                    <p className="font-sans text-xs text-muted-foreground">by {gallery.artistName || "Unknown Artist"}</p>
                  </div>
                </div>
                <CardContent className="p-6">
                  <p className="font-sans text-sm text-muted-foreground line-clamp-3 mb-6 min-h-[60px]">
